...
public class LoadoutResetJob implements Runnable {
	...
	public void run() {
		...
		player.addPotionEffect(PotionEffectType.REGENERATION.createEffect((int)(team.getTeamConfig().resolveInt(TeamConfig.SPAWNPROTECTIONTIME) * 20L), (team.getTeamConfig().resolveInt(TeamConfig.SPAWNPROTECTIONPOTENCY))));
